...
import { resetPass } from '@/app/store/auth/auth-action';
...
export default function ResetPassword() {
    ...
    const { resetStatus, resetErr } = useSelector(state => state.auth);
            ...
            <section className={c.resetPass}>
                ...
                <div className={`${c.left}`}>
                    ...
                    <div className={`${c.formcontent}`} data-aos="fade-up" data-aos-offset="200" data-aos-delay="0" data-aos-duration="200">
                        ...
                        <p>Create a new password for your ExpenseVision account</p>
                        ...
                        <Formik
                            ...
                            onSubmit={(values, { setSubmitting, resetForm }) => {
                                dispatch(resetPass({ token, password: values.password }))
                                    .unwrap()
                                    .then(() => {
                                        // setMessageSent(true);
                                        resetForm();
                        ...
                        >
                            {({isSubmitting}) => (
                                <Form className={c.form}>
                                    ...
                                    {resetStatus === 'failed' && resetErr && (
                                        <div className={c.generalError}>{resetErr}</div>
                                    )}

                                    <button type="submit" className={c.submit} disabled={isSubmitting || resetStatus === 'pending'} >
                                        {isSubmitting ?
                                            (<>
                                                <CircleLoader /> Resetting password
                                            </>) : (<>
                                                <Lock strokeWidth={1.75} size={16} /> Reset password
                                            </>)}
                                    </button>
                                    ...
                                    <div className={c.directLogin}>
                                        <Link href={`/login`}>Remembered your password? <span>Sign in</span></Link>
                                    </div>
